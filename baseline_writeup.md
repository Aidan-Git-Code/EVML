...
**Phase C, 16 threads, post cache-wipe (2026-04-18T01:37 → ~15:07, ~13h30m).** Wiped Go's persistent fuzz cache and relaunched. Baseline replay shrunk from 2437 entries (~14s, poisoned) to the 255 built-in byte patterns (~6s, clean). 466 supervisor cycles, mean FAIL cycle 84.8s (shorter than phase A because replay parallelizes across 16 workers), 4228 new-interesting entries, 989,892 state-test JSONs written to `out/baseline/`. Total ≈ 215.8 CPU-hours of substrate (wall-clock × threads). The supervisor ran ~1h past its scheduled stop at 14:02:28; the detached kill-timer didn't fire (cause uninvestigated; not load-bearing for the baseline artifact). It eventually died on its own around 15:07, at which point the crasher watcher detected the dead pid and self-exited.
...
- **What we'll measure:** cumulative new-interesting rate over time (phase C: ~4,228 / 13.5 h = ~313 / h; LLM target: beat this, especially past hour 6 when baseline plateaus), plus the actual goal: unique cross-client divergences per CPU-hour once goevmlab is wired (Day 4).
