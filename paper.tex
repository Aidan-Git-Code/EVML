...
fuzzillai \cite{fuzzillai} adds an agentic LLM layer to Fuzzilli, using an LLM to propose program plans and an ``evolve by generating'' plateau-recovery mechanism. The plateau-recovery idea, the retrieval-augmented objective selection, and the structured-plan output format are adapted from fuzzillai. Several components were deliberately not adapted. The PostgreSQL-backed distributed corpus is unnecessary for a single-machine six-day project. gdb-based crash triage does not apply because EVM differential runs produce text diffs rather than native crashes: a consensus flaw is a state-root mismatch between two processes that both exited zero, not a segfault. Hosted LLM APIs were avoided to keep the system fully offline and reproducible without per-call cost. The present system runs entirely on-device on a 16 GB-VRAM consumer GPU.
...
Over the 13h30m window, the loop driver completed \NUM{llm-batches} batches and generated \NUM{llm-tests-total} state-tests. Throughput averaged \NUM{llm-tests-per-cpuh} tests per CPU-hour, against a baseline of \NUM{baseline-tests-per-cpuh} tests per CPU-hour over the same wall-clock window. The LLM plan-generation step added \NUM{llm-plan-overhead-pct} percent overhead to each batch's wall-clock, measured as plan-decode duration divided by total batch duration.

The plateau detector triggered \NUM{llm-rotations} objective rotations during the run. Each rotation inspects the last $K=3$ diff reports; the first few rotations in this run occurred early because the LLM-guided path was operating under \texttt{--no-diff} and diff reports are not produced inline. The rotations that did fire were driven by the driver's start-time heuristic and by manually seeded prior objectives; in a diff-enabled run these would be driven by true zero-divergence plateaus. This behavior is documented as a known limitation of the \texttt{--no-diff} mode.
...
\textbf{Metric} & \textbf{Value} \\
\hline
State-tests generated & \NUM{llm-tests-total} \\
Tests with non-empty contract code & \NUM{llm-tests-with-code} \\
Unique bytecode hashes (SHA-256) & \NUM{llm-unique-bc} \\
Corpus dedup rate & \NUM{llm-dedup} \\
Total opcodes disassembled & \NUM{llm-total-ops} \\
Mean opcodes per coded test & \NUM{llm-mean-ops} \\
Unique opcode 3-grams & \NUM{llm-n3} \\
Unique opcode 5-grams & \NUM{llm-n5} \\
\hline
\end{tabular}
\end{table}
...
\textbf{Divergences per CPU-hour.} The baseline produced \NUM{baseline-div} divergences over 215.8 CPU-hours, a rate of \NUM{baseline-div-per-cpuh} divergences per CPU-hour. The LLM-guided run produced \NUM{llm-div} divergences over \NUM{llm-cpuh} CPU-hours, a rate of \NUM{llm-div-per-cpuh} divergences per CPU-hour. The ratio \NUM{div-rate-ratio} expresses the LLM-guided rate as a multiple of the baseline rate.

\textbf{Unique opcode-sequence coverage.} The baseline achieved 272{,}408 unique 3-grams and 656{,}511 unique 5-grams. The LLM-guided run achieved \NUM{llm-n3} unique 3-grams (\NUM{n3-ratio} of baseline) and \NUM{llm-n5} unique 5-grams (\NUM{n5-ratio} of baseline). Lower $n$-gram diversity is expected for the LLM-guided path when the objectives target narrow EIP regions; higher diversity would suggest the biasing is not successfully concentrating the distribution.

\textbf{Corpus-dedup rate.} The baseline's dedup rate was 0.534 (528{,}212 unique bytecode bodies of 989{,}892 generated tests). The LLM-guided rate is \NUM{llm-dedup} (\NUM{llm-unique-bc} unique bodies of \NUM{llm-tests-total} tests). A dedup rate closer to 1.0 indicates more diverse output; a rate closer to the baseline's 0.534 indicates similar redundancy despite the biasing.

\textbf{Plan-to-divergence attribution.} Of the \NUM{llm-plans-total} plans executed during the run, \NUM{llm-plans-with-div} produced at least one divergence and \NUM{llm-plans-zero-div} produced none. Table~\ref{tab:top_objectives} lists the objectives that produced the most divergences; these are the objectives that the plateau detector should keep or combine with, while zero-div objectives are candidates for rotation. The most productive objectives in this run were \NUM{top-objective-1} (\NUM{top-objective-1-count} divergences) and \NUM{top-objective-2} (\NUM{top-objective-2-count} divergences). Divergences broke down by client pair as \NUM{vm-pair-breakdown} (geth versus revm).

\begin{table}[t]
\centering
\caption{Top five objectives by divergence count in the LLM-guided run. Objectives that produced zero divergences are omitted but counted toward the plateau-rotation analysis.}
\label{tab:top_objectives}
\begin{tabular}{lrr}
\hline
\textbf{Objective} & \textbf{Tests} & \textbf{Divergences} \\
\hline
\NUM{obj-row-1} \\
\NUM{obj-row-2} \\
\NUM{obj-row-3} \\
...
\NUM{discussion-summary-one-line} summarizes the outcome in one sentence. The expected pattern, if LLM guidance is useful at this CPU budget, is a higher divergences-per-CPU-hour rate for the LLM-guided arm combined with lower $n$-gram diversity, because the LLM concentrates generation into a narrower region that is more likely to surface bugs. The null hypothesis is the opposite: equal or higher diversity on the baseline and equal or higher divergence rate on the baseline, which would suggest the LLM biasing adds overhead without improving targeted coverage. The observed numbers fall in the \NUM{outcome-category} regime. Plausible reasons and their evidence are discussed below.

The \NUM{llm-plans-zero-div} zero-divergence plans account for \NUM{zero-div-tests-pct} percent of the LLM-guided test budget. This is the strongest single signal in the comparison: if zero-div plans dominate, the plateau detector is either not firing often enough, or the LLM is not generating sufficiently distinct objectives after a rotation. Both are addressable in future work without changing the pipeline structure.
...
\bibitem{fuzzyvm} M. van der Wijden, ``Introducing FuzzyVM,'' \url{https://mariusvanderwijden.github.io/blog/2021/05/02/FuzzyVM/}, 2021.

\bibitem{goevmlab} M. Holiman, ``goevmlab,'' \url{https://github.com/holiman/goevmlab}.

\bibitem{fuzzilli} S. Gro{\ss}, ``Fuzzilli: Coverage-Guided Fuzzing for JavaScript Engines,'' M.S. thesis, Saarland University, 2018.

\bibitem{fuzzillai} VRIG-RITSEC, ``fuzzillai: Agentic LLM layer over Fuzzilli,'' \url{https://github.com/VRIG-RITSEC/fuzzillai}, 2024.

\bibitem{differential} T. Petsios \textit{et al.}, ``Differential Testing for Software,'' arXiv:1903.08483, 2019.

\bibitem{rag} P. Lewis \textit{et al.}, ``Retrieval-Augmented Generation for Knowledge-Intensive NLP Tasks,'' \textit{Advances in Neural Information Processing Systems}, 2020.

\bibitem{bge} S. Xiao \textit{et al.}, ``C-Pack: Packaged Resources to Advance General Chinese Embedding,'' arXiv:2309.07597, 2023.

\bibitem{faiss} J. Johnson, M. Douze, and H. J{\'e}gou, ``Billion-Scale Similarity Search with GPUs,'' \textit{IEEE Transactions on Big Data}, vol. 7, no. 3, pp. 535--547, 2019.

\bibitem{llamacpp} G. Gerganov \textit{et al.}, ``llama.cpp,'' \url{https://github.com/ggerganov/llama.cpp}.

\bibitem{qwencoder} Qwen Team, ``Qwen2.5-Coder Technical Report,'' arXiv:2409.12186, 2024.

\bibitem{guidance} B. T. Willard and R. Louf, ``Efficient Guided Generation for Large Language Models,'' arXiv:2307.09702, 2023.

\bibitem{eip1153} Ethereum Foundation, ``EIP-1153: Transient Storage Opcodes,'' \url{https://eips.ethereum.org/EIPS/eip-1153}.

\bibitem{eip2929} Ethereum Foundation, ``EIP-2929: Gas Cost Increases for State Access Opcodes,'' \url{https://eips.ethereum.org/EIPS/eip-2929}.

\bibitem{ethbounty} Ethereum Foundation, ``Bug Bounty Program,'' \url{https://bounty.ethereum.org/}.
