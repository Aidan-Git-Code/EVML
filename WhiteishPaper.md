...
**Plan loader.** FuzzyVM now accepts a plan at `--plan path.json` or via `FUZZYVM_PLAN=...` env var. Loaded at generator `init()`; overrides the `strategies` map, active fork, and banned-opcode set. Stock weights remain the default when no plan is present, so baseline behavior is untouched. Strategy-name validation up-front rejects unknown names with the full known-strategy list in the error string, so LLM misspellings fail loud instead of silently no-op'ing.
...
- The upstream seed corpus contains at least one input (seed#56 byte pattern) that segfaults geth's EVM through cgo. It bypasses Go's `recover` and takes down the whole `go test` process. Mitigation: move `FuzzyVM/corpus/` aside to `corpus_disabled/` so `readCorpus()` sees an empty set and the fuzzer falls back to the 255 built-in byte-pattern seeds.
